  ...
  try {
    switch (s_action) {
      case 'star': {
        ...
        const s_key = `star:${s_repo.full_name}`;
        
        if (g_actionHistory.has(s_key)) {
          return { success: false, action: 'star', reason: 'Already starred in session' };
        }
        
        const s_result = await m_starRepo(s_repo.owner.login, s_repo.name);
        if (s_result) g_actionHistory.add(s_key);
      ...
      case 'fork': {
        const s_repo = s_repos[Math.floor(Math.random() * s_repos.length)];
        const s_key = `fork:${s_repo.full_name}`;
        
        if (g_actionHistory.has(s_key)) {
          return { success: false, action: 'fork', reason: 'Already forked in session' };
        }
      ...
      case 'watch': {
        const s_repo = s_repos[Math.floor(Math.random() * s_repos.length)];
        const s_key = `watch:${s_repo.full_name}`;
        
        if (g_actionHistory.has(s_key)) {
          return { success: false, action: 'watch', reason: 'Already watching in session' };
        }
      ...
      case 'follow': {
        ...
        const s_key = `follow:${s_user}`;
        
        if (g_actionHistory.has(s_key)) {
          return { success: false, action: 'follow', reason: 'Already followed in session' };
        }
...
const m_runSession = async () => {
  ...
  const s_topicCount = Math.floor(Math.random() * 2) + 2;
  const s_topics = [];
  while (s_topics.length < s_topicCount) {
    const s_topic = m_getRandomTopic();
    if (!s_topics.includes(s_topic)) {
      s_topics.push(s_topic);
    }
  }
  ...
  if (s_repos.length === 0) {
    console.warn('No repos found, ending session early');
    return {
      duration: Date.now() - s_startTime,
      total: 0,
      successful: 0,
      results: []
    };
  ...
  while (m_shouldContinue(s_startTime, s_actionCount)) {
    ...
    if (s_result.success) {
      ...
      s_failureStreak++;
      if (s_failureStreak >= 5) {
        console.warn('Too many failures, ending session');
        break;
      }
    }
  }
